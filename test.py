...
b=c[0]*c[0]*np.sqrt(Sigma[0]/(Alfa[0]*(c[0]**2)*Sigma[0]+(c[0]**2)*Sigma[0]-(a**2)*Alfa[0]*Sigma[0]-2*P_max[0]*(c[0]**2))))
y=(a**2)*(np.exp(-Attenuation_length[0,1]*(c[0]-a)))*(np.exp(-Attenuation_length[0,0]*(OD-c[0])))
fig5 = plt.figure()
plt.plot(a,y)
